...
class nowcity(models.Model):  #疫情所有城市的最新数据
    ...
    unique_together = (("province", "city"),)  #联合主键
    # province= models.ForeignKey('alldaycity', to_field='province', on_delete=models.CASCADE)   #外键
    class Meta:
        #managed = False
        db_table = 'nowcity'
class nowprovince(models.Model):  #疫情所有城市的最新数据
    province= models.CharField(db_column='province', max_length=255, blank=True)  # Field name made lowercase.
    # city = models.CharField(db_column='city', max_length=255, blank=True)  # Field name made lowercase.
    confirmedCount = models.CharField(db_column='confirmedCount', max_length=255, blank=True, null=True)  # Field name made lowercase.
    deadCount= models.CharField(db_column='deadCount', max_length=255, blank=True, null=True)  # Field name made lowercase.
...
class weibo(models.Model):  #疫情微博评论据
    weiboid= models.TextField(db_column='weiboid', max_length=2550, blank=True, null=True)  # Field name made lowercase.
    content = models.TextField(db_column='content', max_length=10000, blank=True, null=True)  # Field name made lowercase.
...
class alldayprovince(models.Model):  #疫情各省份的数据，从2020.1.23  截至目前
    province= models.CharField(db_column='province', max_length=255, blank=True)  # Field name made lowercase.
    date= models.CharField(db_column='date', max_length=255, blank=True)  # Field name made lowercase.
    ...
    # unique_together = (("province", "date"),)  #联合主键
    # province= models.ForeignKey('nowcity', to_field='province', on_delete=models.CASCADE)   #外键
    class Meta:
        #managed = False
        db_table = 'alldayprovince'
class migratehuibei(models.Model):  #人口迁移，
